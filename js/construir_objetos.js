...
var arrayCars = []; //array para almacenar los objetos
//con un bucle flor vamos instanciando nuevos objetos a traves del constructor
for (let i = 0; i < carsName.length; i++) {
    var car = new coche(carsName[i], carsModel[i], carsYear[i], carsColor[i]); //declaramos el nuevo coche
    arrayCars.push(car); //añadimos el objeto coche al array
